function [varargout] = DESR1(input64,mode,key)
...
for Sbox = 1:8
    L{2} = R{1}; % half key: 32-bit
    expended_R = EF(R{1}); % expended half key: 32-bit to 48-bit

    for bit=1:64
        bitguess = guess(bit,:);
        for j=(1+(Sbox-1)*6):6+(Sbox-1)*6
            subKeys(1,j) = str2num(bitguess(j-(Sbox-1)*6));
        end
        mixed_R = KM(expended_R,subKeys(1,:));
        substituted_R = SBOX(mixed_R); % substitution: 48-bit to 32-bit
        permuted_R = PBOX(reshape(substituted_R',1,32)); % permutation: 32-bit
