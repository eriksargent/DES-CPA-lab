%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is the first optimization: Plaintext Optimization.
% Note that findGoodPlaintexts. must be run before running this in order to
% search through the plaintexts and pick out the ones that are the most
% unique.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

disp('Running optimization 1: Plaintext Optimization');
disp('Please make sure findGoodPlaintexts has been run before running this');

% Number of traces to load in
numValues = 800;
% Directory of the traces
folderName = 'goodTraces/';
...
for i = 1:numValues
    ...
    % After 100 rounds of the key not changing, break out of the loop.
    % Keep correlating the key as long as it is still changing
    if ~isequal(prevSubkey, subkey)
        cnt = 0;
    else
        cnt = cnt + 1;
    end
